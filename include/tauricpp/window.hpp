...
public:
    ...
    struct Config {
        ...
        bool borderless = false;        ///< 无边框窗口（无标题栏），需前端自行实现拖拽/关闭/最小化
    ...
    /// 向WebView投递一条消息（线程安全，可由任意线程调用，实际投递在UI线程完成）
    void PostWebMessage(const std::string& json);

    /// 从队列取出并向WebView投递消息（UI线程调用）
    void PostWebMessageFromQueue();
...
private:
    ...
    // 线程安全的WebView消息队列（用于把后台线程的命令结果回投到UI线程）
    std::mutex webMsgQueueMutex_;
    std::queue<std::string> webMsgQueue_;
};
